;(function () {
    ...
    ;(function () {
        ...
        if (myCursor.length && body) {
            ...
            let test = function (ev) {
                if (
                    ev.target.tagName === "A" ||
                    ev.target.className === "cursor-pointer"
                ) {
                    ev.preventDefault()
                    e.classList.toggle("cursor-hover"),
                        t.classList.toggle("cursor-hover")
                }
            }
            ;(window.onmousemove = function (s) {
                ...
                body.addEventListener("mouseenter", test, true),
                body.addEventListener("mouseleave", test, true),
    ...
    ;(function () {
        ...
        function find(str) {
            ...
            db.filter((item) => {
                //第一层
                let tp = []
                item.child.filter((itema) => {
                    //第二层分类
                    itema.child.filter((itemb) => {
                        //第三层主题
                        let arr = []
                        itemb.child &&
                            itemb.child.filter((itemc) => {
                                //文章层
            ...
            if (str.length < 2) {
                ...
                for (let itema of data) {
                    ...
                    for (let itemb of itema.child) {
                        ...
                        da.innerHTML = itemb.title.replace(
                            new RegExp(str, "g"),
                            "<span>" + str + "</span>"
                        )
                        dd.appendChild(da)
                        for (let itemc of itemb.child) {
                            ...
                            ha.href =
                                "/article.html?id=" + itemc.url + "-" + itemb.id
                            ha.innerHTML = itemc.title.replace(
                                new RegExp(str, "g"),
                                "<span>" + str + "</span>"
                            )
    ...
    var Gs = {
        //Notification
        ShowNoti: function (opt) {
            let audio = new Audio("/app/mp3/chime.mp3")
            let obj = {
                dir: "auto", //默认值是auto, 可以是ltr或rtl
                lang: "zh-cmn-Hans",
                tag: "msg-notification",
                renotify: true, //新通知出现是否覆盖旧的通知
                badge: "/app/img/favicon.png",
                icon: "/app/img/jiang.png",
                image: "/app/img/edu/vue3.jpg",
                body: opt.body,
            }
            const init = function (title, opt) {
        ...
        quickMenu: (function () {
            ...
            var quickHTML =
                    '<button class="btn quick-reward">赏<p><img src="/app/img/alipay/wx.jpg"/></p></button><button class="btn quick-top" onclick="Gs.scrollToTop(event)"><i class="ic-up"></i></button>',
        ...
        getData: function (url) {
            ...
            return new Promise(function (resolve, reject) {
                fetch("./app/json/" + url + ".json?t=" + Date.now(), {
                    method: "GET",
                })
                    .then((response) => {
                        if (response.ok) {
        ...
        obj: {
            ...
            initArticle: function () {
                ...
                Gs.getData(url)
                    .then((rs) => {
                        ...
                        rs.data.content = rs.data.content
                            .replace(/<code>/g, "")
                            .replace(/<\/code>/g, "")
                        Gs.$("#articleCon").innerHTML =
                            Gs.tpls.articleContent.tpl(rs.data)
                    })
                    .catch((err) => {
                        ...
                        if (el) {
                            el.innerHTML =
                                '<h2 class="nofile">没有找到相关文章</h2>'
                        }
                    })

                this.getList(pid).then((data) => {
                    if (data) {
                        Gs.$("#articleDesc").innerHTML =
                            Gs.tpls.listDesc.tpl(data)
                        Gs.$("#article-list").innerHTML = this.articleList(data)
                        this.moveScrollTop(data)
                        Gs.$("#article-total").innerHTML =
                            (data.child.length || 0) + "篇"
                    } else {
                        Gs.$("#articleBox").innerHTML =
                            '<h2 class="nofile">错误了,传参有误!!!</h2>'
                    }
                })
            ...
            htmlMenu: function () {
                return new Promise((resolve) => {
                    ...
                    for (let i = 0, len = db.length; i < len; i++) {
                        if (i == 0) {
                            html +=
                                '<dd class="rnav nos cur" onclick="Gs.obj.htmlMenuList(event,' +
                                i +
                                ')">' +
                                db[i].title +
                                "</dd>"
                            ...
                            html +=
                                '<dd class="rnav nos" onclick="Gs.obj.htmlMenuList(event,' +
                                i +
                                ')">' +
                                db[i].title +
                                "</dd>"
            ...
            getMenuList: function (data) {
                return new Promise((resolve) => {
                    ...
                    for (let ra of data.child) {
                        html +=
                            '<dl><dt><a href="javascript:void(0)">' +
                            ra.title +
                            "</a></dt><dd>"
                        for (let rb of ra.child) {
                            if (rb.child) {
            ...
            htmlList: function () {
                return new Promise((resolve) => {
                    ...
                    for (let item of db) {
                        html +=
                            '<dl class="flex-wrap flex-break home-item-list"><dt>' +
                            item.title +
                            "</dt>"
                        for (let ra of item.child) {
                            for (let rb of ra.child) {
                                if (
                                    rb.flag &&
                                    rb.flag.indexOf("recommend") === 0
                                ) {
                                    rb.count =
                                        (rb.child && rb.child.length) || 0
                ...
                return new Promise((resolve) => {
                    ...
                    if (url.args === "study") {
                        ...
                        for (let item of links[url.args]) {
                            ...
                            if (item.id % 3 === 0) {
                                html +=
                                    '</ul><ul class="flex-wrap flex-break study-list">'
                            }
                        }
                    } else if (url.args === "expert") {
                        html +=
                            '<ul class="flex-wrap flex-break study-list expert-list">'
                        for (let item of links[url.args]) {
                            html += Gs.tpls.studyList.tpl(item)
                            if (item.id % 2 === 0) {
                                html +=
                                    '</ul><ul class="flex-wrap flex-break study-list expert-list">'
                            }
                        }
                ...
                if (data.child.length > 0) {
                    for (let [key, item] of Object.entries(data.child)) {
                        if (url.args === item.url + "-" + data.id) {
                            html += `<dd><a class="cur" href="article.html?id=${
                                item.url
                            }-${data.id}">${parseInt(key) + 1}. ${
                                item.title
                            }</a></dd>`
                        } else {
                            html += `<dd><a href="article.html?id=${item.url}-${
                                data.id
                            }">${parseInt(key) + 1}. ${item.title}</a></dd>`
                        }
                    }
                }
                return html
        ...
        pop: {
            ...
            init: function () {
                ...
                var txt =
                    '<div class="alert-ui-bg"></div><div class="alert-ui-cont"><div class="alert-ui-tit">提示</div><div class="alert-ui-msg"></div><div class="alert-ui-btnc"><a class="alert-ui-btn-ok">确定</a></div></div>'
                ...
                if (files.length) {
                    ...
                    reader.onload = function () {
                        ...
                        if (Gs.file.flag) {
                            ...
                            Gs.file.editor.customConfig.menus = [
                                "head",
                                "bold",
                                "italic",
                                "underline",
